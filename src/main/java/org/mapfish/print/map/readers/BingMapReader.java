 ...
 */
public class BingMapReader extends TileableMapReader {
    public static class Factory implements MapReaderFactory {
        @Override
        public List<? extends MapReader> create(String type, RenderingContext context,
                PJsonObject params) {
            return Collections.singletonList(new BingMapReader("t", context, params));
        }
    }

    protected final String layer;

    protected BingMapReader(String layer, RenderingContext context, PJsonObject params) {
    ...
    protected URI getTileUri(URI commonUri, Transformer transformer, double minGeoX, double minGeoY, double maxGeoX, double maxGeoY, long w, long h) throws URISyntaxException, UnsupportedEncodingException {
        ...
        String quadkey = this.tile2quad(tileX, tileY-1, Integer.parseInt(String.format("%02d", resolution.index)));
        path.append(quadkey);
        path.append('.').append(tileCacheLayerInfo.getExtension());

        URI uri = new URI(commonUri.getScheme(), commonUri.getUserInfo(), commonUri.getHost(), commonUri.getPort(), commonUri.getPath() + path, "g=3299", commonUri.getFragment());

        return new URI(commonUri.getScheme(), commonUri.getUserInfo(), commonUri.getHost(), commonUri.getPort(), commonUri.getPath() + path, "g=3299", commonUri.getFragment());
    }
    //for bing maps
